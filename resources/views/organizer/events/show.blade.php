...
<div class="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 sm:py-8">
    ...
    <section class="rounded-2xl border bg-white p-4 sm:p-5" x-data="{ inviteRole: 'staff' }">
        ...
        <div class="mt-4 grid gap-5 lg:grid-cols-2">
            <div><h3 class="text-sm font-medium">使用 Email 加入</h3><form method="POST" action="{{ route('organizer.events.staff.store',$event) }}" class="mt-3 grid gap-3 sm:grid-cols-[minmax(0,1fr)_9rem_auto]">@csrf<input type="email" name="email" required class="min-h-11 min-w-0 w-full rounded-xl border-gray-300 text-base sm:text-sm" placeholder="已註冊會員 Email"><select name="role" class="min-h-11 w-full rounded-xl border-gray-300 text-base sm:text-sm"><option value="manager">管理者</option><option value="staff">工作人員</option><option value="judge">裁判</option><option value="chief_judge">主裁判</option><option value="volunteer">志工</option><option value="viewer">只讀</option></select><button class="min-h-11 rounded-xl bg-gray-900 px-4 text-sm text-white">加入</button></form></div>
            <div class="rounded-2xl bg-indigo-50 p-4"><h3 class="font-semibold text-indigo-950">掃描 QR Code 加入</h3><p class="mt-1 text-sm text-indigo-700">選擇身分後，讓成員掃描並登入確認。邀請 24 小時後失效。</p><label class="mt-3 block text-sm font-medium text-indigo-950">加入身分<select x-model="inviteRole" class="mt-1 min-h-11 w-full rounded-xl border-indigo-200 bg-white"><option value="manager">管理者</option><option value="staff">工作人員</option><option value="judge">裁判</option><option value="chief_judge">主裁判</option><option value="volunteer">志工</option><option value="viewer">只讀</option></select></label><div class="mt-3 flex justify-center rounded-xl bg-white p-3">@foreach($staffInviteQrs as $role => $qr)<img x-show="inviteRole === '{{ $role }}'" src="{{ $qr }}" alt="{{ $role }} 工作團隊邀請 QR Code" class="h-52 w-52 max-w-full">@endforeach</div></div>
        </div>
        <div class="mt-5 divide-y">@foreach($event->staff as $staff)<div class="flex items-center justify-between gap-3 py-3 text-sm"><div class="min-w-0"><p class="break-words font-medium">{{ $staff->user?->display_name }}</p><p class="text-xs text-gray-500">{{ ['owner'=>'擁有者','manager'=>'管理者','staff'=>'工作人員','judge'=>'裁判','chief_judge'=>'主裁判','volunteer'=>'志工','viewer'=>'只讀'][$staff->role] ?? $staff->role }}・{{ $staff->status }}</p></div>@if($staff->role!=='owner' && $staff->status==='active')<form method="POST" action="{{ route('organizer.events.staff.revoke',[$event,$staff]) }}">@csrf @method('PATCH')<button class="min-h-11 rounded-xl px-3 text-xs text-red-600">撤銷</button></form>@endif</div>@endforeach</div>
    </section>
    @endcan
