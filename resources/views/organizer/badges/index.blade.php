...
<div class="mx-auto max-w-6xl space-y-7 px-4 py-8 sm:px-6">
    ...
    <section class="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        <form method="POST" action="{{ route('organizer.events.badges.store', $event) }}" enctype="multipart/form-data" class="mt-5 grid gap-4 md:grid-cols-2" x-data="{ type: '{{ old('type', 'participant') }}', rule: '{{ old('award_rule', 'attendance') }}', roles: {{ Illuminate\Support\Js::from(old('staff_roles', ['owner','manager','staff'])) }}, counts: {{ Illuminate\Support\Js::from($teamCounts) }}, changeType(){ if(this.type==='staff') this.rule='staff'; else if(this.type==='volunteer') this.rule='volunteer'; else if(['staff','volunteer'].includes(this.rule)) this.rule=this.type==='special'?'manual':'attendance' }, get previewCount(){ if(this.type==='volunteer') return Number(this.counts.volunteer||0); if(this.type==='staff') return this.roles.reduce((sum,role)=>sum+Number(this.counts[role]||0),0); return 0 } }">
            @csrf
            ...
            <div x-show="rule !== 'staff' && rule !== 'volunteer'"><label class="text-sm font-medium">適用條件</label><select name="eligibility" class="mt-1 w-full rounded-xl border-gray-300"><option value="registered">已有有效報名</option><option value="checked_in">已完成報到</option><option value="scored">已有有效成績</option><option value="any">不限資格</option></select></div>
            <div><label class="text-sm font-medium">取得方式 *</label><template x-if="type==='staff'"><p class="mt-1 rounded-xl bg-indigo-50 p-3 text-sm text-indigo-700">加入指定工作身分後自動取得</p></template><template x-if="type==='volunteer'"><p class="mt-1 rounded-xl bg-indigo-50 p-3 text-sm text-indigo-700">成為賽事志工後自動取得</p></template><select x-show="type!=='staff' && type!=='volunteer'" :disabled="type==='staff' || type==='volunteer'" x-model="rule" class="mt-1 w-full rounded-xl border-gray-300"><option value="attendance">繳費並報到後自動取得</option><option value="placement">正式成績名次自動取得</option><option value="manual">主辦方賽後授予</option></select><input type="hidden" name="award_rule" :value="rule"></div>
