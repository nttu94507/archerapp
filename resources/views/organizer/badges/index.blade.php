...
<div class="mx-auto max-w-6xl space-y-7 px-4 py-8 sm:px-6">
    ...
    <section class="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        <form method="POST" action="{{ route('organizer.events.badges.store', $event) }}" enctype="multipart/form-data" class="mt-5 grid gap-4 md:grid-cols-2" x-data="{ type: '{{ old('type', 'participant') }}', rule: '{{ old('award_rule', $event->requiresCheckIn() ? 'attendance' : 'manual') }}', roles: {{ Illuminate\Support\Js::from(old('staff_roles', ['owner','manager','staff'])) }}, counts: {{ Illuminate\Support\Js::from($teamCounts) }}, changeType(){ if(this.type==='staff') this.rule='staff'; else if(this.type==='volunteer') this.rule='volunteer'; else if(['staff','volunteer'].includes(this.rule)) this.rule=this.type==='special'?'manual':'{{ $event->requiresCheckIn() ? 'attendance' : 'manual' }}' }, get previewCount(){ if(this.type==='volunteer') return Number(this.counts.volunteer||0); if(this.type==='staff') return this.roles.reduce((sum,role)=>sum+Number(this.counts[role]||0),0); return 0 } }">
            @csrf
