<section class="space-y-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 sm:p-5">
    <div><h2 class="font-bold text-amber-950">{{ $reason }}・加射</h2><p class="mt-1 text-sm text-amber-800">雙方各射 1 箭；同分時由主裁判依距離靶心遠近判定。</p></div>
    ...
    <div class="overflow-hidden rounded-xl border border-amber-200 bg-white">
        ...
        <div class="grid grid-cols-[3rem_1fr_1fr] items-center gap-2 border-b px-3 py-3 text-center text-sm last:border-b-0">
            <strong>#{{ $shootOff->attempt_number }}</strong><span>{{ $match->participantOneEntry->athlete_name }}：<strong>{{ $shootOff->participant_one_arrow }}</strong></span><span>{{ $match->participantTwoEntry->athlete_name }}：<strong>{{ $shootOff->participant_two_arrow }}</strong></span>
    ...
    <form method="POST" action="{{ route('organizer.events.elimination.matches.shoot-offs.adjudicate', [$event, $match]) }}" class="space-y-3">@csrf
        <div class="grid gap-2 sm:grid-cols-3"><label class="flex min-h-12 items-center gap-2 rounded-xl border border-amber-200 bg-white px-3"><input type="radio" name="decision" value="participant_one" required> {{ $match->participantOneEntry->athlete_name }}較近</label><label class="flex min-h-12 items-center gap-2 rounded-xl border border-amber-200 bg-white px-3"><input type="radio" name="decision" value="participant_two" required> {{ $match->participantTwoEntry->athlete_name }}較近</label><label class="flex min-h-12 items-center gap-2 rounded-xl border border-amber-200 bg-white px-3"><input type="radio" name="decision" value="re_shoot" required> 同距離，重新加射</label></div>
