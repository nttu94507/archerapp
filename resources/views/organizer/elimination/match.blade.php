...
@section('content')
@php
    $canEnter = in_array($match->status, ['ready', 'in_progress'], true) && $match->sets->count() < 5;
...
<div class="mx-auto flex min-h-[calc(100dvh-4rem)] max-w-5xl flex-col gap-4 bg-gray-50 px-3 py-4 sm:px-5">
    ...
    <section class="grid grid-cols-[minmax(0,1fr)_4rem_minmax(0,1fr)] items-stretch overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div class="p-4 text-center"><span class="text-xs font-bold text-gray-400">種子 {{ $match->participant_one_seed }}</span><h2 class="mt-1 truncate font-bold sm:text-lg">{{ $match->participantOneEntry->athlete_name }}</h2><p class="mt-3 text-4xl font-black text-indigo-700">{{ $match->participant_one_set_points }}</p></div>
        <div class="flex items-center justify-center border-x bg-gray-50 text-sm font-bold text-gray-400">局分</div>
        <div class="p-4 text-center"><span class="text-xs font-bold text-gray-400">種子 {{ $match->participant_two_seed }}</span><h2 class="mt-1 truncate font-bold sm:text-lg">{{ $match->participantTwoEntry->athlete_name }}</h2><p class="mt-3 text-4xl font-black text-indigo-700">{{ $match->participant_two_set_points }}</p></div>
    </section>

    @if($match->sets->isNotEmpty())
    <section class="overflow-hidden rounded-2xl border bg-white shadow-sm"><div class="grid grid-cols-[3rem_minmax(0,1fr)_4rem_minmax(0,1fr)_4rem] bg-gray-50 px-3 py-2 text-center text-xs font-semibold text-gray-500"><span>局</span><span>{{ $match->participantOneEntry->athlete_name }}</span><span>總分</span><span>{{ $match->participantTwoEntry->athlete_name }}</span><span>總分</span></div>@foreach($match->sets as $set)<div class="grid min-h-12 grid-cols-[3rem_minmax(0,1fr)_4rem_minmax(0,1fr)_4rem] items-center border-t px-3 text-center text-sm"><strong>{{ $set->set_number }}</strong><span>{{ implode('・', $set->participant_one_arrows) }}</span><strong>{{ $set->participant_one_total }}</strong><span>{{ implode('・', $set->participant_two_arrows) }}</span><strong>{{ $set->participant_two_total }}</strong></div>@endforeach</section>
    @endif

    @if(in_array($match->status, ['awaiting_shoot_off', 'awaiting_judge'], true))
        @include('organizer.elimination._shoot-off', ['reason'=>'五局後 5：5'])
    ...
    <form method="POST" action="{{ route('organizer.events.elimination.matches.sets.store', [$event, $match]) }}" id="set-form" class="grid gap-3">@csrf
