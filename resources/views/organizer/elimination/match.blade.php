@extends('layouts.app')
@section('title', $match->label.' 局分計分')
@section('content')
@php
...
<div class="mx-auto flex min-h-[calc(100dvh-4rem)] max-w-5xl flex-col gap-4 bg-gray-50 px-3 py-4 sm:px-5">
    ...
    <section class="grid grid-cols-[minmax(0,1fr)_4rem_minmax(0,1fr)] items-stretch overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div class="p-4 text-center"><span class="text-xs font-bold text-gray-400">種子 {{ $match->participant_one_seed }}</span><h2 class="mt-1 truncate font-bold sm:text-lg">{{ $participantOneName }}</h2><p class="mt-3 text-4xl font-black text-indigo-700">{{ $match->participant_one_set_points }}</p></div>
        <div class="flex items-center justify-center border-x bg-gray-50 text-sm font-bold text-gray-400">局分</div>
        <div class="p-4 text-center"><span class="text-xs font-bold text-gray-400">種子 {{ $match->participant_two_seed }}</span><h2 class="mt-1 truncate font-bold sm:text-lg">{{ $participantTwoName }}</h2><p class="mt-3 text-4xl font-black text-indigo-700">{{ $match->participant_two_set_points }}</p></div>
    </section>

    @if($match->sets->isNotEmpty())
    <section class="overflow-hidden rounded-2xl border bg-white shadow-sm"><div class="grid grid-cols-[3rem_minmax(0,1fr)_4rem_minmax(0,1fr)_4rem] bg-gray-50 px-3 py-2 text-center text-xs font-semibold text-gray-500"><span>局</span><span>{{ $participantOneName }}</span><span>總分</span><span>{{ $participantTwoName }}</span><span>總分</span></div>@foreach($match->sets as $set)<div class="grid min-h-12 grid-cols-[3rem_minmax(0,1fr)_4rem_minmax(0,1fr)_4rem] items-center border-t px-3 text-center text-sm"><strong>{{ $set->set_number }}</strong><span>{{ implode('・', $set->participant_one_arrows) }}</span><strong>{{ $set->participant_one_total }}</strong><span>{{ implode('・', $set->participant_two_arrows) }}</span><strong>{{ $set->participant_two_total }}</strong></div>@endforeach</section>
    @endif

    @if(in_array($match->status, ['awaiting_shoot_off', 'awaiting_judge'], true))
        @include('organizer.elimination._shoot-off', ['reason'=>$teamMatch ? '四局後 4：4' : '五局後 5：5'])
    ...
    <form method="POST" action="{{ route('elimination-stations.sets.store', $stationToken) }}" id="set-form" class="grid gap-3">@csrf
        <section class="rounded-2xl border bg-white p-3 shadow-sm"><div class="mb-3 text-center text-sm font-bold text-indigo-700">第 {{ $match->sets->count() + 1 }} 局 / 最多 {{ $maxSets }} 局</div><div class="divide-y rounded-xl border">
            @foreach([['participant_one_arrows', $participantOneName], ['participant_two_arrows', $participantTwoName]] as [$field, $name])
            <div class="athlete-row grid grid-cols-[minmax(6rem,1fr)_minmax(10rem,1.2fr)_3rem] items-center gap-2 p-3"><strong class="truncate text-sm">{{ $name }}</strong><div class="grid gap-2" style="grid-template-columns: repeat({{ $arrowCount }}, minmax(0, 1fr))">@for($arrow=0;$arrow<$arrowCount;$arrow++)<input readonly inputmode="none" name="{{ $field }}[]" placeholder="＿" class="score-input h-11 min-w-0 cursor-pointer select-none rounded-lg border-gray-300 p-1 text-center text-lg font-bold placeholder:text-gray-300 focus:ring-2 focus:ring-indigo-500">@endfor</div><strong class="row-total text-right text-xl">0</strong></div>
            @endforeach
        </div></section>
        <section class="rounded-2xl border bg-white p-3 shadow-sm"><div class="grid grid-cols-4 gap-2">@foreach([['X','X','bg-yellow-50 border-yellow-200'],['10','10','bg-yellow-50 border-yellow-200'],['9','9','bg-yellow-50 border-yellow-200'],['BKSP','⌫','bg-white border-gray-300'],['8','8','bg-red-50 border-red-200'],['7','7','bg-red-50 border-red-200'],['6','6','bg-blue-50 border-blue-200'],['5','5','bg-blue-50 border-blue-200'],['4','4','bg-gray-100 border-gray-300'],['3','3','bg-gray-100 border-gray-300'],['2','2','bg-white border-gray-300'],['1','1','bg-white border-gray-300']] as [$key,$label,$color])<button type="button" data-key="{{ $key }}" class="score-key min-h-14 touch-manipulation rounded-xl border text-lg font-bold {{ $color }}">{{ $label }}</button>@endforeach<button type="button" data-key="M" class="score-key col-span-2 min-h-14 rounded-xl border border-green-200 bg-green-50 text-lg font-bold text-green-800">M</button><button type="button" data-key="SUBMIT" class="score-key col-span-2 min-h-14 rounded-xl bg-indigo-600 font-semibold text-white">送出本局</button></div></section>
    </form>
    @endif
    @endif
</div>
