...
@php
    ...
    $maxEnds = $teamMatch ? 4 : 5;
    $canEnter = in_array($match->status, ['ready', 'in_progress'], true) && $match->ends->count() < $maxEnds;
...
<div class="mx-auto flex min-h-[calc(100dvh-4rem)] max-w-5xl flex-col gap-4 bg-gray-50 px-3 py-4 sm:px-5">
    ...
    <section class="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div class="grid grid-cols-[3rem_minmax(0,1fr)_3.5rem_minmax(0,1fr)_3.5rem] bg-gray-50 px-3 py-2 text-center text-xs font-semibold text-gray-500"><span>趟</span><span>{{ $participantOneName }}</span><span>小計</span><span>{{ $participantTwoName }}</span><span>小計</span></div>
    ...
    @if(in_array($match->status, ['awaiting_shoot_off', 'awaiting_judge'], true))
        @include('organizer.elimination._shoot-off', ['reason'=>$teamMatch ? '四趟累計同分' : '十五箭累計同分'])
    ...
    <form method="POST" action="{{ route('elimination-stations.ends.store', $stationToken) }}" id="end-form" class="grid gap-3">@csrf
        <section class="rounded-2xl border bg-white p-3 shadow-sm"><div class="mb-3 text-center text-sm font-bold text-indigo-700">第 {{ $match->ends->count() + 1 }} 趟 / {{ $maxEnds }} 趟</div><div class="divide-y rounded-xl border">
