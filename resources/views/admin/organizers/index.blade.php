...
@php($statusClasses = ['pending'=>'bg-yellow-100 text-yellow-800','legacy_review'=>'bg-yellow-100 text-yellow-800','approved'=>'bg-green-100 text-green-700','changes_requested'=>'bg-yellow-100 text-yellow-800','rejected'=>'bg-red-100 text-red-700','suspended'=>'bg-red-100 text-red-700','draft'=>'bg-gray-100 text-gray-600'])
        ...
        @forelse($profiles as $profile)
            <article class="rounded-2xl border bg-white p-4 shadow-sm">
                <div class="flex min-w-0 items-start justify-between gap-3">
                    ...
                    <span class="shrink-0 rounded-full px-2.5 py-1 text-xs font-medium {{ $statusClasses[$profile->status] ?? 'bg-gray-100 text-gray-600' }}">{{ $statusLabels[$profile->status] ?? $profile->status }}</span>
                </div>
    ...
    <div class="hidden overflow-hidden rounded-2xl border bg-white sm:block">
        <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y">
                @forelse($profiles as $profile)
                    <tr><td class="p-4 font-medium">{{ $profile->organization_name }}</td><td class="p-4">{{ $profile->user?->display_name }}<p class="text-xs text-gray-500">{{ $profile->contact_email }}</p></td><td class="p-4"><span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $statusClasses[$profile->status] ?? 'bg-gray-100 text-gray-600' }}">{{ $statusLabels[$profile->status] ?? $profile->status }}</span></td><td class="p-4">{{ $profile->applications_count }}</td><td class="p-4 text-right"><a href="{{ route('admin.organizers.show',$profile) }}" class="inline-flex min-h-10 items-center rounded-lg border px-4 text-xs font-medium">審核</a></td></tr>
