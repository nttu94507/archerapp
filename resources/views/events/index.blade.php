...
@php
    ...
    $statusMeta = [
        'ongoing'=>['label'=>'比賽中', 'class'=>'bg-emerald-100 text-emerald-700'],
        'registration_open'=>['label'=>'報名中', 'class'=>'bg-indigo-100 text-indigo-700'],
        'registration_closed'=>['label'=>'報名截止', 'class'=>'bg-gray-200 text-gray-700'],
        'upcoming'=>['label'=>'即將開放', 'class'=>'bg-amber-100 text-amber-800'],
    ];
@endphp
<main class="mx-auto max-w-6xl space-y-8 px-4 py-6 sm:px-6 sm:py-8">
    ...
    <section class="space-y-4">
        <div class="flex items-end justify-between gap-3"><h2 class="text-lg font-semibold text-gray-900">近期賽事</h2><span class="text-xs text-gray-400">{{ $featuredEvents->count() }} 場</span></div>
        ...
        @else
            <div class="grid gap-4 md:grid-cols-2">
                @foreach($featuredEvents as $event)
                    ...
                    @endphp
                    <a href="{{ $actionUrl }}" class="flex min-h-44 flex-col rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-300 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2">
                        ...
                        <div class="mt-3 min-w-0 flex-1"><h3 class="break-words text-lg font-bold text-gray-900">{{ $event->name }}</h3><p class="mt-1 text-sm text-gray-600">{{ $dateRange($event) }}</p><p class="mt-1 truncate text-sm text-gray-500">{{ $event->venue ?: '場地待公布' }}・{{ $event->organizer }}</p>
                            @if($event->listing_status === 'registration_open')<p class="mt-2 text-xs font-medium text-indigo-700">截止 {{ $event->registrationClosesAt()?->format('Y-m-d H:i') ?? '依組別公告' }}</p>@endif
                        </div>
                    </a>
                @endforeach
            </div>
    ...
    <section class="space-y-4">
        <div class="flex items-end justify-between gap-3"><h2 class="text-lg font-semibold text-gray-900">歷史賽事</h2><span class="text-xs text-gray-400">{{ $pastEvents->count() }} 場</span></div>
        ...
        @else
            <div class="grid gap-3 md:grid-cols-2">
                @foreach($historyPreview as $event)
                    <a href="{{ route('events.show', $event) }}" class="flex min-h-28 items-center rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition hover:border-indigo-300 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2"><div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><span class="rounded-full {{ $event->isOfficiallyCompleted() ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600' }} px-2 py-1 text-xs font-medium">{{ $event->isOfficiallyCompleted() ? '已結束' : '成績整理中' }}</span><span class="text-xs text-gray-400">{{ $event->mode === 'indoor' ? '室內' : '室外' }}</span></div><h3 class="mt-2 truncate font-semibold text-gray-900">{{ $event->name }}</h3><p class="mt-1 text-xs text-gray-500">{{ $dateRange($event) }}・{{ $event->venue ?: '場地待公布' }}</p></div></a>
                @endforeach
            </div>
            @if($historyRemaining->isNotEmpty())
                <details class="group rounded-2xl border border-gray-200 bg-white p-4"><summary class="flex min-h-11 cursor-pointer list-none items-center justify-between font-medium"><span>查看其餘 {{ $historyRemaining->count() }} 場歷史賽事</span><span class="text-gray-400 transition group-open:rotate-180">⌄</span></summary><div class="mt-3 grid gap-3 border-t pt-4 md:grid-cols-2">@foreach($historyRemaining as $event)<a href="{{ route('events.show', $event) }}" class="rounded-xl border border-gray-200 bg-gray-50 p-4 transition hover:border-indigo-300"><div class="flex items-center justify-between gap-2"><p class="font-medium text-gray-800">{{ $event->name }}</p><span class="shrink-0 rounded-full {{ $event->isOfficiallyCompleted() ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-200 text-gray-600' }} px-2 py-1 text-xs font-medium">{{ $event->isOfficiallyCompleted() ? '已結束' : '成績整理中' }}</span></div><p class="mt-1 text-xs text-gray-500">{{ $dateRange($event) }}</p></a>@endforeach</div></details>
            @endif
        @endif
    </section>
</main>
