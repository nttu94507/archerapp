...
<div class="mx-auto max-w-2xl px-4 py-6 sm:px-6 sm:py-10">
    ...
    <div class="mt-2 rounded-2xl border bg-white p-5 shadow-sm sm:p-7">
        ...
        <dl class="mt-6 divide-y rounded-xl bg-gray-50 px-4">
            ...
            @php($registrationCapacity = $event->isFreePlan() ? 16 : $group->quota)
            <div class="flex justify-between gap-4 py-3"><dt class="text-sm text-gray-500">目前名額</dt><dd class="text-right text-sm font-semibold">{{ $registrationCapacity ? $registered.' / '.$registrationCapacity.' 人' : '無上限' }}</dd></div>
        ...
        <form method="POST" action="{{ route('events.quick_register', [$event, $group]) }}" class="mt-6">
            @csrf
            @if($group->hasTeamFormat('mixed'))<div class="mb-5 rounded-xl bg-violet-50 p-4 text-sm text-violet-800">本組別開放男女混雙。競賽性別：<strong>{{ auth()->user()->profile?->gender === 'male' ? '男子' : '女子' }}</strong>（依會員資料帶入）</div>@endif
