...
<main class="mx-auto max-w-6xl space-y-6 px-4 py-6 sm:px-6 sm:py-8">
    <header>
        ...
        <p class="mt-1 text-sm text-gray-500">{{ collect([$group->hasTeamFormat('standard') ? '三人團體（登記4人、取前三高）' : null,$group->hasTeamFormat('mixed') ? '男女混雙（固定2人）' : null])->filter()->join('＋') }}・{{ $group->teamFormationIsOpen() ? '目前開放組隊' : '組隊已截止，名單已鎖定' }}</p>
    </header>
    ...
    @if($myRegistration && !$myMembership && $group->teamFormationIsOpen())
        <section class="rounded-2xl border border-indigo-200 bg-indigo-50 p-4 sm:p-5">
            ...
            <form method="POST" action="{{ route('events.teams.store',[$event,$group]) }}" class="mt-4 grid gap-3 sm:grid-cols-2">@csrf
                <select name="team_format" required class="min-h-12 rounded-xl border-indigo-200 bg-white"><option value="">選擇參加形式</option>@if($group->hasTeamFormat('standard'))<option value="standard">三人團體（登記4人）</option>@endif @if($group->hasTeamFormat('mixed'))<option value="mixed">男女混雙（2人）</option>@endif</select>
                ...
                <input name="recruitment_note" maxlength="300" value="{{ old('recruitment_note') }}" class="min-h-12 min-w-0 rounded-xl border-indigo-200 sm:col-span-1" placeholder="招募說明（選填，例如：尋找同校隊友）">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            @forelse($teams as $team)
                @php
                    ...
                    $mixedEligible = $team->team_format !== 'mixed' || ($myRegistration?->athlete_gender && ! $activeGenders->contains($myRegistration->athlete_gender));
                    $wantedGender = $team->team_format === 'mixed' ? ($activeGenders->contains('male') ? '女性' : ($activeGenders->contains('female') ? '男性' : '一男一女')) : null;
                    $requiredSize = $team->requiredSize();
                @endphp
                <article class="rounded-2xl border bg-white p-4 shadow-sm sm:p-5">
                    <div class="flex items-start justify-between gap-3"><div><div class="flex flex-wrap items-center gap-2"><h3 class="font-bold">{{ $team->name }}</h3><span class="rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-semibold text-violet-700">{{ $team->team_format === 'mixed' ? '男女混雙・2人' : '三人團體・4人名單' }}</span>@if($team->status==='recruiting' && $team->is_open)<span class="rounded-full bg-indigo-100 px-2 py-0.5 text-[10px] font-semibold text-indigo-700">公開招募</span>@endif</div><p class="mt-1 text-xs text-gray-500">隊長：{{ $team->captainRegistration?->name }}</p></div><span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $competingCount >= $requiredSize ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }}">{{ $competingCount }} / {{ $requiredSize }}</span></div>
    ...
    @if($rankings->isNotEmpty())
        <section class="overflow-hidden rounded-2xl border bg-white shadow-sm"><div class="border-b p-4 sm:p-5"><h2 class="text-lg font-semibold">團體正式排名</h2><p class="mt-1 text-sm text-gray-500">三人團體由4人名單取個人成績最高3人；混雙計算兩位選手。</p></div><div class="overflow-x-auto"><table class="min-w-full text-sm"><thead class="bg-gray-50 text-left text-xs text-gray-500"><tr><th class="p-3">排名</th><th class="p-3">隊伍</th><th class="p-3 text-right">總分</th><th class="p-3 text-right">10</th><th class="p-3 text-right">X</th></tr></thead><tbody class="divide-y">@foreach($rankings as $index=>$row)<tr><td class="p-3 font-bold">{{ $index+1 }}</td><td class="p-3"><strong>{{ $row['team']->name }}</strong><p class="mt-1 text-xs text-gray-500">@foreach($row['team']->memberships->where('status','active') as $member)<span class="mr-2 {{ in_array($member->id,$row['counted_member_ids'],true) ? 'font-semibold text-emerald-700' : 'text-gray-400' }}">{{ $member->registration?->name }}{{ in_array($member->id,$row['counted_member_ids'],true) ? '・計分' : '・替補' }}</span>@endforeach</p></td><td class="p-3 text-right text-lg font-bold">{{ $row['total'] }}</td><td class="p-3 text-right">{{ $row['ten_count'] }}</td><td class="p-3 text-right">{{ $row['x_count'] }}</td></tr>@endforeach</tbody></table></div></section>
    @endif
</main>
@endsection
