...
@section('content')
    <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-8">
        <section class="rounded-2xl border bg-white p-5 sm:p-6">
            ...
            <p class="mt-2 text-sm text-gray-600">販售者暱稱會自動帶入你的帳號名稱。</p>
            ...
            <form action="{{ route('second-hand.store') }}" method="POST" enctype="multipart/form-data" class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <label class="space-y-1">
                    ...
                    <input type="number" min="0" name="price" value="{{ old('price') }}" required class="w-full rounded-xl border px-3 py-2 text-sm">
                </label>
                <label class="space-y-1">
                ...
                <label class="space-y-1">
                    ...
                    <select name="contact_type" class="w-full rounded-xl border px-3 py-2 text-sm" required>
                        <option value="phone" @selected(old('contact_type') === 'phone')>手機</option>
                        <option value="social" @selected(old('contact_type') === 'social')>社群媒體</option>
                    </select>
                </label>
                <label class="space-y-1">
                    <span class="text-sm text-gray-700">聯繫方式</span>
                    <input name="contact_value" value="{{ old('contact_value') }}" required class="w-full rounded-xl border px-3 py-2 text-sm" placeholder="例如：0912xxxxxx / IG: archer123">
                </label>
                <label class="space-y-1 md:col-span-2">
                    <span class="text-sm text-gray-700">補充說明（選填）</span>
                    <textarea name="description" rows="3" class="w-full rounded-xl border px-3 py-2 text-sm">{{ old('description') }}</textarea>
                </label>
                <div class="md:col-span-2 flex gap-2">
                    ...
                    <a href="{{ route('second-hand.index') }}" class="inline-flex items-center justify-center rounded-xl border px-5 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">返回總覽</a>
                </div>
            </form>
        </section>
    </div>
