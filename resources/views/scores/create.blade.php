...
// 如果沒有 thead/tbody 就沒辦法生成
            ...
            ready(() => {
                ...
                function showZoom(nx, ny, label, persist = false, opts = {}) {
                    ...
                    // Always keep the lens above the finger; allow it to overflow past the target/body top if needed
                    const desiredAbove = clientY - lensOffsetY;
                    const minLensY = -lensHeight * 0.35; // permit partial overshoot above the viewport
                    const maxLensY = viewportH - lensHeight / 2 - 4;
                    const lensY = clamp(desiredAbove, minLensY, maxLensY);
