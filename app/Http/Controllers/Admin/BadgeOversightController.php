...
class BadgeOversightController extends Controller
{
    public function index(Request $request): View
    {
        ...
        return view('admin.badges.index', compact('badges','source','sourceCounts'));
    }
    ...
    {
        $data=$request->validate(['name'=>['required','string','max:120'],'description'=>['nullable','string','max:1000'],'max_supply'=>['nullable','integer','min:1'],'icon'=>['nullable','image','mimes:jpg,jpeg,png,webp','max:10240'],'location_claim_enabled'=>['nullable','boolean'],'claim_lat'=>['nullable','required_if:location_claim_enabled,1','numeric','between:-90,90'],'claim_lng'=>['nullable','required_if:location_claim_enabled,1','numeric','between:-180,180'],'claim_radius_km'=>['nullable','numeric','between:1,50'],'claim_period'=>['nullable','in:unlimited,range'],'claim_start_date'=>['nullable','required_if:claim_period,range','date'],'claim_end_date'=>['nullable','required_if:claim_period,range','date','after_or_equal:claim_start_date']]);
